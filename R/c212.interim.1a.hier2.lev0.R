...
{
	interim = M_global$INTERIMdata(Mi_1a_h2_l0, trial.data, iter, nchains, burnin, initial_values)
	...
	sim.params = M_global$INTERIM_sim_params1a(Mi_1a_h2_l0, sim.params, sim_type, trial.data, cntrl.data)
	...
	Ret2 = .Call("c2121a_interim_hier2_exec", as.integer(nchains), as.integer(burnin),
					...
					sim.params,
					monitor,
					as.integer(Mi_1a_h2_l0$numIntervals), as.integer(Mi_1a_h2_l0$level),
					Mi_1a_h2_l0$maxBs, as.integer(Mi_1a_h2_l0$numB),
					as.integer(Mi_1a_h2_l0$maxAEs),
	...
	model_fit = list(id = Mi_1a_h2_l0$Id,
			...
			chains = nchains,
			nIntervals = Mi_1a_h2_l0$numIntervals,
			Intervals = Mi_1a_h2_l0$Intervals,
			nBodySys = Mi_1a_h2_l0$numB,
			maxBs = Mi_1a_h2_l0$maxBs,
			maxAEs = Mi_1a_h2_l0$maxAEs,
			nAE = Mi_1a_h2_l0$nAE,
			AE=Mi_1a_h2_l0$AE,
			B = Mi_1a_h2_l0$B,
			burnin = burnin,
...
Mi_1a_h2_l0$initVars = function() {
    ...
    Mi_1a_h2_l0$B <- c()
    Mi_1a_h2_l0$numB <- NA
    Mi_1a_h2_l0$numIntervals <- NA
    Mi_1a_h2_l0$nAE <- c()
    Mi_1a_h2_l0$maxAEs <- NA
...
Mi_1a_h2_l0$initChains = function(c) {
	...
	for (i in 1:Mi_1a_h2_l0$numIntervals) {
		numB = Mi_1a_h2_l0$numB[i]
		for (b in 1:numB) {
			Mi_1a_h2_l0$gamma[c, i, b, 1:Mi_1a_h2_l0$nAE[i, b]] <-
										runif(Mi_1a_h2_l0$nAE[i, b], -10, 10)
			Mi_1a_h2_l0$theta[c, i, b, 1:Mi_1a_h2_l0$nAE[i, b]] <-
										runif(Mi_1a_h2_l0$nAE[i, b], -10, 10)

			Mi_1a_h2_l0$theta[c, i, b, ][is.infinite(Mi_1a_h2_l0$theta[c, i, b, ])] =
										-10
		...
		Mi_1a_h2_l0$mu.gamma[c, i, 1:numB] = runif(numB, -10, 10)
		Mi_1a_h2_l0$mu.theta[c, i, 1:numB] = runif(numB, -10, 10)
		Mi_1a_h2_l0$sigma2.gamma[c, i, 1:numB] = runif(numB, 5, 20)
		Mi_1a_h2_l0$sigma2.theta[c, i, 1:numB] = runif(numB, 5, 20)
	}
}

Mi_1a_h2_l0$initialiseChains = function(initial_values, nchains) {

	Mi_1a_h2_l0$theta = array(0, dim=c(nchains, Mi_1a_h2_l0$numIntervals,
											Mi_1a_h2_l0$maxBs, Mi_1a_h2_l0$maxAEs))
	Mi_1a_h2_l0$gamma = array(0, dim=c(nchains, Mi_1a_h2_l0$numIntervals,
											Mi_1a_h2_l0$maxBs, Mi_1a_h2_l0$maxAEs))
	...
	if (is.null(initial_values)) {
		...
		for (i in 1:Mi_1a_h2_l0$numIntervals) {
			...
			for (b in 1:numB) {
				...
				Mi_1a_h2_l0$theta[1, i, b, ][is.nan(Mi_1a_h2_l0$theta[1, i, b, ])] = -10 # -1000
				Mi_1a_h2_l0$gamma[1, i, b, ][is.nan(Mi_1a_h2_l0$gamma[1, i, b, ])] = -10 # -1000
			}
		}

		Mi_1a_h2_l0$mu.gamma <- array(0, dim = c(nchains, Mi_1a_h2_l0$numIntervals,
													Mi_1a_h2_l0$maxBs))
		Mi_1a_h2_l0$mu.theta <- array(0, dim = c(nchains, Mi_1a_h2_l0$numIntervals,
													Mi_1a_h2_l0$maxBs))
		Mi_1a_h2_l0$sigma2.gamma <- array(10, dim = c(nchains, Mi_1a_h2_l0$numIntervals,
													Mi_1a_h2_l0$maxBs))
		Mi_1a_h2_l0$sigma2.theta <- array(10, dim = c(nchains, Mi_1a_h2_l0$numIntervals,
													Mi_1a_h2_l0$maxBs))
		...
		for (c in 1:nchains) {
			for (i in 1:Mi_1a_h2_l0$numIntervals) {
				interval = Mi_1a_h2_l0$Intervals[i]
		...
		Mi_1a_h2_l0$mu.theta <- array(0, dim = c(nchains, Mi_1a_h2_l0$numIntervals,
													Mi_1a_h2_l0$maxBs))
		Mi_1a_h2_l0$sigma2.gamma <- array(0, dim = c(nchains, Mi_1a_h2_l0$numIntervals,
													Mi_1a_h2_l0$maxBs))
		Mi_1a_h2_l0$sigma2.theta <- array(0, dim = c(nchains, Mi_1a_h2_l0$numIntervals,
													Mi_1a_h2_l0$maxBs))

		for (c in 1:nchains) {
			for (i in 1:Mi_1a_h2_l0$numIntervals) {
				interval = Mi_1a_h2_l0$Intervals[i]
				for (b in 1:Mi_1a_h2_l0$numB[i]) {
					data = initial_values$mu.gamma[initial_values$mu.gamma$chain ==
									c & initial_values$mu.gamma$Interval == interval
									& initial_values$mu.gamma$B == Mi_1a_h2_l0$B[i, b],]
					Mi_1a_h2_l0$mu.gamma[c, i, b] = data$value

					data = initial_values$mu.theta[initial_values$mu.theta$chain ==
									c & initial_values$mu.theta$Interval == interval
									& initial_values$mu.theta$B == Mi_1a_h2_l0$B[i, b],]
					Mi_1a_h2_l0$mu.theta[c, i, b] = data$value

					data = initial_values$sigma2.gamma[initial_values$sigma2.gamma$chain == c &
									initial_values$sigma2.gamma$Interval == interval
									& initial_values$sigma2.gamma$B == Mi_1a_h2_l0$B[i, b],]
					...
					data = initial_values$sigma2.theta[initial_values$sigma2.theta$chain == c &
									...
									& initial_values$sigma2.theta$B == Mi_1a_h2_l0$B[i, b],]
					Mi_1a_h2_l0$sigma2.theta[c, i, b] = data$value
				}
			}
		}
		...
		for (c in 1:nchains) {
			for (i in 1:Mi_1a_h2_l0$numIntervals) {
				interval = Mi_1a_h2_l0$Intervals[i]
				for (b in 1:Mi_1a_h2_l0$numB[i]) {
					for (j in 1:Mi_1a_h2_l0$nAE[i, b]) {
						ae = Mi_1a_h2_l0$AE[i, b, j]
						data = initial_values$gamma[initial_values$gamma$chain == c
										& initial_values$gamma$Interval == interval
										& initial_values$gamma$B == Mi_1a_h2_l0$B[i, b]
						...
						data = initial_values$theta[initial_values$theta$chain == c
										...
										& initial_values$theta$B == Mi_1a_h2_l0$B[i, b]
										& initial_values$theta$AE == ae,]
						Mi_1a_h2_l0$theta[c, i, b, j] = data$value
					}
				}
			}
		}
