...
Id <- "$Id: c212.interim.1a.hier3.lev2.convergence.R,v 1.9 2019/05/05 13:18:12 clb13102 Exp clb13102 $"

c212.interim.1a.dep.lev2.convergence.diag <- function(raw, debug_diagnostic = FALSE)
{
	if (is.null(raw)) {
		print("NULL raw data")
		return(NULL)
	...
	if (nchains > 1) {
		...
		for (i in 1:raw$nIntervals) {
			for (b in 1:raw$nBodySys[i]) {
				for (j in 1:raw$nAE[i, b]) {
					# theta
					if (theta_mon == 1) {
						g = M_global$Geweke(raw$theta[, i, b, j, ])
						row <- data.frame(Interval = raw$Intervals[i], B = raw$B[i, b],
		...
		if (tau2.theta.0_mon == 1) {
			...
			row <- data.frame(stat = g$z, upper_ci = NA)
...
c212.interim.1a.dep.lev2.print.convergence.summary <- function(conv) {
