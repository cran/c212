...
Id <- "$Id: c212.interim.1a.hier3.lev0.summary.stats.R,v 1.9 2019/05/05 13:18:12 clb13102 Exp clb13102 $"

c212.interim.1a.indep.summary.stats <- function(raw)
{
	if (is.null(raw)) {
		print("NULL raw data");
		return(NULL)
	...
	sigma2.theta_summ = data.frame(interval = character(0), B = character(0), mean = numeric(0),
													hpi_lower = numeric(0),
													hpi_upper = numeric(0), SD = numeric(0), SE = numeric(0))
	mu.gamma.0_summ = data.frame(interval = character(0), mean = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	mu.theta.0_summ = data.frame(interval = character(0), mean = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	tau2.theta.0_summ = data.frame(interval = character(0), mean = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	tau2.gamma.0_summ = data.frame(interval = character(0), mean = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	...
	for (i in 1:raw$nIntervals) {
		# mu.gamma.0
		if (mu.gamma.0_mon == 1) {
			s = M_global$summaryStats(raw$mu.gamma.0[, i, ], nchains)
		...
		# mu.theta.0
		if (mu.theta.0_mon == 1) {
			s = M_global$summaryStats(raw$mu.theta.0[, i, ], nchains)
			row <- data.frame(interval = raw$Intervals[i], mean = s[1],
							hpi_lower = s[3], hpi_upper = s[4],
							SD = s[5], SE = s[6])
		...
		if (tau2.gamma.0_mon == 1) {
			s = M_global$summaryStats(raw$tau2.gamma.0[, i, ], nchains)
			row <- data.frame(interval = raw$Intervals[i], mean = s[1],
							hpi_lower = s[3], hpi_upper = s[4],
							SD = s[5], SE = s[6])
		...
		if (tau2.theta.0_mon == 1) {
			s = M_global$summaryStats(raw$tau2.theta.0[, i, ], nchains)
			row <- data.frame(interval = raw$Intervals[i], mean = s[1],
							hpi_lower = s[3], hpi_upper = s[4],
							SD = s[5], SE = s[6])
...
c212.interim.1a.indep.print.summary.stats <- function(summ)
{
	if (is.null(summ)) {
