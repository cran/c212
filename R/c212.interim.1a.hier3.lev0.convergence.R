...
c212.interim.1a.indep.convergence.diag <- function(raw, debug_diagnostic = FALSE)
{
	...
	sigma2.theta_conv = data.frame(Interval = character(0), B = character(0),
						stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	mu.gamma.0_conv = data.frame(Interval = character(0), stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	mu.theta.0_conv = data.frame(Interval = character(0), stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	tau2.theta.0_conv = data.frame(Interval = character(0), stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	tau2.gamma.0_conv = data.frame(Interval = character(0), stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	...
	if (nchains > 1) {
		...
		if (mu.theta.0_mon == 1) {
			for (i in 1:raw$nIntervals) {
				g = M_global$GelmanRubin(raw$mu.theta.0[, i, ], nchains)
				row <- data.frame(Interval = raw$Intervals[i], stat = g$psrf[1], upper_ci =  g$psrf[2], stringsAsFactors=FALSE)
		...
		if (tau2.theta.0_mon == 1) {
			for (i in 1:raw$nIntervals) {
				g = M_global$GelmanRubin(raw$tau2.theta.0[, i, ], nchains)
				row <- data.frame(Interval = raw$Intervals[i], stat = g$psrf[1], upper_ci =  g$psrf[2], stringsAsFactors=FALSE)
		...
		for (i in 1:raw$nIntervals) {
			...
			if (mu.theta.0_mon == 1) {
				g = M_global$Geweke(raw$mu.theta.0[1, i, ])
				row <- data.frame(Interval = raw$Intervals[i], stat = g$z, upper_ci = NA, stringsAsFactors=FALSE)
				mu.theta.0_conv = rbind(mu.theta.0_conv, row)
			}
			...
			if (tau2.theta.0_mon == 1) {
				g = M_global$Geweke(raw$tau2.theta.0[1, i, ])
				row <- data.frame(Interval = raw$Intervals[i], stat = g$z, upper_ci = NA, stringsAsFactors=FALSE)
				tau2.theta.0_conv = rbind(tau2.theta.0_conv, row)
			}
		}
...
c212.interim.1a.indep.print.convergence.summary <- function(conv) {
