...
Id <- "$Id: c212.interim.BB.hier3.lev2.summary.stats.R,v 1.7 2016/10/14 10:39:05 clb13102 Exp clb13102 $"

c212.interim.BB.dep.lev2.summary.stats <- function(raw)
{
	s_base = c212.interim.1a.dep.lev2.summary.stats(raw)
	...
	alpha.pi_summ = data.frame(mean = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	beta.pi_summ = data.frame(mean = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
...
c212.interim.BB.dep.lev2.print.summary.stats <- function(summ)
{
	if (is.null(summ)) {
	...
	if (alpha_pi_mon == 1)
		for (i in 1:nrow(summ$alpha.pi.summary)) {
			...
			cat(sprintf("alpha.pi: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n",
					row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	if (beta_pi_mon == 1)
		for (i in 1:nrow(summ$beta.pi.summary)) {
			row = summ$beta.pi.summary[i, ]
			cat(sprintf("beta.pi: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n",
					row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
}
